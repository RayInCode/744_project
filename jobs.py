...
class _TFJobs(object):
    ...
    def predict_completed_jobs(self) -> list:
        predict_comp_jobs = list()
        for rjob in self.runnable_jobs:
            if 'RUNNING' == rjob['status']:
                remaining_time = rjob['remaining_iteration']*rjob['iteration_time_cur']
                if(remaining_time <= FLAGS.round_interval):
                    predict_comp_jobs.append(rjob)
        return predict_comp_jobs
    
    def pop_arriving_jobs(self, time) -> list:
        arriving_jobs = list()
        if len(self.job_events) > 0 and self.job_events[0]['time'] <= time:
            arriving_jobs = self.job_events[0]['start_jobs']
            self.job_events.pop(0)
        return arriving_jobs

    # def handle_completed_jobs(self, event: dict, cluster_state: _Cluster, LOG: _Log):
    ...
    #             LOG.job_complete(e_job, event['time'])
    #             self.runnable_jobs.remove(e_job)
